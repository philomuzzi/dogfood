...
bool ParseTest_C(shared_ptr<GamePlayer> self, const void *msg, const short msglen) {
	std::string str(static_cast<const char*>(msg));
	cout << "msg len: " << msglen << endl;
	cout << "msg content: " << str << endl;
//	self->write(str, msglen);

	cout << "player size: " << sizeof(GamePlayer) << endl;
	return true;
}

bool ParseLogic_C(shared_ptr<GamePlayer> self, const void *msg, const short msglen) {
	LoginGame_C rev;
...
bool ParseEnterGame_CSS(shared_ptr<GamePlayer> self, const void *msg, const short msglen) {
	EnterGame_CSS rev;
	rev.ParsePartialFromArray(msg, msglen);
